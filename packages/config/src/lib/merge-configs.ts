import {
  ...
  isObject,
  simpleDeepClone,
} from '@cometx-server/common';

import { CometXConfig, PartialCometXConfig } from './config.interface';
/**
 * @description
 * Performs a deep merge of two CometXConfig objects. Unlike `Object.assign()` the `target` object is
 ...
 */
export function mergeConfig<T extends CometXConfig>(
  target: T,
  source: PartialCometXConfig,
  depth = 0,
): T {
  if (!source) {
    return target;
